...
public class StudentDaoImpl implements StudentDao {
	...
	public Student getStudentById(long id) {
		...
		try (Connection con = ConnectionFactory.getMySqlConnection();
			...
			if (rs.next()) {
				return new Student(rs.getLong("id"), rs.getString("name"), rs.getLong("mobileNo"),
						rs.getString("gender").charAt(0), rs.getFloat("gradPerc"), rs.getDouble("salary"),
						rs.getBoolean("isMember"), rs.getDate("dob").toLocalDate(),
	...
	public List<Student> getAllStudents() {
		...
		try (Connection con = ConnectionFactory.getMySqlConnection();
			...
			while (rs.next()) {
				students.add(new Student(rs.getLong("id"), rs.getString("name"), rs.getLong("mobileNo"),
						rs.getString("gender").charAt(0), rs.getFloat("gradPerc"), rs.getDouble("salary"),
						rs.getBoolean("isMember"), rs.getDate("dob").toLocalDate(),
						rs.getTimestamp("joining").toLocalDateTime()));
			}
